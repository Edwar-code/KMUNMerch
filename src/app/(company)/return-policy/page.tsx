...
const ReturnsPolicyPage = () => {
  const [returnsPolicy, setReturnsPolicy] = useState<Policy | null>(null);
  ...
  useEffect(() => {
    const fetchReturnsPolicy = async () => {
      setIsLoading(true); 
      try {
        const returnsResponse = await fetch('/api/policies?type=return');
        if (!returnsResponse.ok) {
          throw new Error(`HTTP error! status: ${returnsResponse.status}`);
        }
        const returnsData = await returnsResponse.json();
        setReturnsPolicy(returnsData[0] || null);
      } catch (error) {
              ...
              <>
                <div className="mb-4">
                  <p className="font-light">
                    <strong>Last Updated:</strong> {formatDate(returnsPolicy.updatedAt)}
                  </p>
                  <p className="font-light">
                    <strong>Created At:</strong> {formatDate(returnsPolicy.createdAt)}
                  </p>
                </div>
                <div className="policy-content" dangerouslySetInnerHTML={{ __html: returnsPolicy.content }} />
              </>
            ) : (
              <p>Returns Policy content not found.</p>
            )}
          </section>
        </div>
      </div>
